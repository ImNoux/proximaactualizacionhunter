 ...
 document.addEventListener('DOMContentLoaded', function() {
  ...
  let isLiked = thread.likes && thread.likes['user_id']; // Replace 'user_id' with actual user ID if you have authentication
  newThread.innerHTML = `
  <div class="thread-date">${thread.timestamp}</div>
  <h2>${thread.title}</h2>
  <p><strong>Categoría:</strong> ${thread.category}</p>
  <p>${thread.description}</p>
  <button class="like-button ${isLiked ? 'liked' : ''}" data-thread-id="${key}">
